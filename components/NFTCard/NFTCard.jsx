...
export default function NFTCard({
	...
	async function getNFTData() {
		...
		const collImgURI = await fetch(
			collURI.replace("ipfs://", "https://ipfs.io/ipfs/")
		)
			.then((res) => res.json())
	...
	async function buyNFT() {
		if (seller == account) {
			...
			try {
				const buyNFTTx = await MarketplaceContract.buyItem(
					nftAddress,
					tokenId,
					{ value: price }
				)
	...
	async function handleRemoveNFT() {
		if (window.ethereum !== "undefined") {
			...
			try {
				const removeNFTTx = await MarketplaceContract.cancelListing(
					nftAddress,
					tokenId
				)
	...
	async function handleUpdateNFT() {
		if (window.ethereum !== "undefined") {
			...
			try {
				...
				const updateNFTTx = await MarketplaceContract.updateListing(
					nftAddress,
					tokenId,
					weiPrice
				)
	...
	return (
		<div
			className={styles["apio__NFTCard"]}
			onMouseEnter={() => {
				setCollectNFT(true)
			}}
			onMouseLeave={() => {
		...
		>
			...
			<div className={styles["apio__NFTCard--text"]}>
				{type !== "userListing" ? (
					<h3
						className={styles["apio__NFTCard--text--name"]}
					>{`${tokenName} #0${tokenId}`}</h3>
				) : (
					<div className={styles["apio__NFTCard--text--user_options"]}>
						<h3
							className={styles["apio__NFTCard--text--user_options--name"]}
						>{`${tokenName} #0${tokenId}`}</h3>
						<button
							className={styles["apio__NFTCard--removeNFT"]}
				...
				{type !== "userListing" ? (
					<p className={styles["apio__NFTCard--creator"]}>
						owner:{" "}
						{seller == account ? "you" : "@" + truncateStr(seller || "", 12)}
					</p>
				) : (
					<p className={styles["apio__NFTCard--creator"]}>
				...
				<div className={styles["apio__NFTCard--text--price"]}>
					...
					<p>
						<FontAwesomeIcon
							...
							className={styles["apio__NFTCard--eth_icon"]}
						/>
					</p>
				</div>
				{type !== "userListing" ? (
					<div
						className={
							collectNFT
								? styles["apio__NFTCard--collectNFT"]
								: styles["hide_collectNFT"]
						}
					>
					...
					<div
						className={
							collectNFT
								? styles["apio__NFTCard--updateNFT"]
								: styles["hide_updateNFT"]
						}
					>
